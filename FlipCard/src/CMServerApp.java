...
public class CMServerApp {
	private CMServerStub m_serverStub;  //레퍼런스 변수 선언, 레퍼런스 타입의 변수, 이 타입의 객체 생성필요.
	...
	public CMServerApp()  
	{
		m_serverStub = new CMServerStub();	//CM의 서버 Stub  ,CM의 서버스텁의 객체 m_serverStub
	...
	public static void main(String[] args) {  //메인, stub모듈을 통해 Interaction, CM API >> Stub 객체(sever or clinet)
		// TODO Auto-generated method stub
		CMServerApp server = new CMServerApp();
		CMServerStub cmStub = server.getServerStub();		//severstub의 객체를 만들자 cmstub을 이용.
		cmStub.setAppEventHandler(server.getServerEventHandler());  //Application level의 event handler 사용, Callback 함수 호출, 상위레이어>하위레이어, 이 경우 CM에서 상위 Application 호출
		//Callback 함수
		server.startCM(); //start CM API 호출, CM 여러개의 스레드로 구성., STUB객체의 메소드를 호출
